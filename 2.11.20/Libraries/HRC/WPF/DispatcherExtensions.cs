...
namespace HRC.WPF
{
    public static class DispatcherExtensions
    {
        ...
        {
            if (!dispatcher.CheckAccess()) dispatcher.Invoke(priority, action);
            else action();
        }
        ...
        {
            if (!dispatcher.CheckAccess()) dispatcher.Invoke(DispatcherPriority.Normal, action);
            else action();
        }
        ...
        public static void InvokeInBackgroundIfRequired(
            ...
            Action action)
        {
            if (!dispatcher.CheckAccess()) dispatcher.Invoke(DispatcherPriority.Background, action);
            else action();
        }
