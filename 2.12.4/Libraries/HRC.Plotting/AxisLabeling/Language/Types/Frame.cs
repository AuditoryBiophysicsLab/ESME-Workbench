...
namespace HRC.Plotting.AxisLabeling.Language.Types
{
    ...
    public class Frame : Value
    {
        ...
        {
            if (!(a.NumColumns == 0 || b.NumColumns == 0 || a.NumRows == b.NumRows)) throw new InvalidOperationException("Cbind only works on frames with the same number of rows");
            return new Frame { Columns = a.Columns.Concat(b.Columns).ToList(), Symbols = a.Symbols.Concat(b.Symbols).ToList(), numRows = Math.Max(a.numRows, b.NumRows) };
        }
        ...
        public Frame Project(IEnumerable<Symbol> symbols)
        {
            ...
            foreach (var s in symbols)
            {
                ...
                if (i < 0) throw new InvalidOperationException("Tried to project non-existent column " + s.Name);
        ...
        public Frame Project(params Symbol[] symbols) { return Project(symbols.AsEnumerable()); }

        public List<T> Select<T>(int i) { return Columns[i].Select<T>(); }
    }
