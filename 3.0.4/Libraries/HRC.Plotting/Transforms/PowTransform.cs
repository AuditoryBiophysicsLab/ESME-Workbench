...
namespace HRC.Plotting.Transforms
{
    ...
    public class PowTransform : GeneralTransform
    {
        ...
        {
            if (double.IsInfinity(xBase) || xBase <= 0) throw new ArgumentException("xBase must be NaN (no change) or a positive number");
            if (double.IsInfinity(yBase) || yBase <= 0) throw new ArgumentException("yBase must be NaN (no change) or a positive number");
            XBase = xBase;
            YBase = yBase;
        }
