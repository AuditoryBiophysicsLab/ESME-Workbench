...
namespace HRC.Plotting.Transforms
{
    /// <summary>
    ...
    public class LogTransform : GeneralTransform
    {
        ...
        {
            if (double.IsInfinity(xBase) || xBase <= 0 || xBase == 1) throw new ArgumentException("xBase must be NaN (no change) or a positive number (excluding 1)");
            if (double.IsInfinity(yBase) || yBase <= 0 || yBase == 1) throw new ArgumentException("yBase must be NaN (no change) or a positive number (excluding 1)");
            XBase = xBase;
            YBase = yBase;
        }
