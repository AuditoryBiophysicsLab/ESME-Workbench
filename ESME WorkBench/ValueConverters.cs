...
namespace ESMEWorkBench
{
    ...
    public class ProgressBarBrushConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            brush2.Viewport = brush2.Viewbox = new Rect(0, 0, d, num2);
            ...
            if (flag)
            {
                ...
                var num9 = -num8 * num7;
                var num10 = d * 0.3;
                brush2.Viewport = brush2.Viewbox = new Rect(num9, 0, num10 - num9, num2);
                ...
                while ((x + width) < num10)
                {
                    context.DrawRectangle(brush, null, new Rect(num9 + x, 0, width, num2));
                    x += num7;
                }
                d = num10;
                x = 0;
            ...
            while ((x + width) < d)
            {
                context.DrawRectangle(brush, null, new Rect(x, 0, width, num2));
                x += num7;
            }
            var num14 = d - x;
            if ((!flag && (num14 > 0)) && (Math.Abs(d - num3) < 1E-05))
            {
                context.DrawRectangle(brush, null, new Rect(x, 0, num14, num2));
            }
            context.Close();
