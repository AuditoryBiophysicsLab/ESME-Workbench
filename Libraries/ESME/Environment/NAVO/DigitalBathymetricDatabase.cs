...
namespace ESME.Environment.NAVO
{
    public class DigitalBathymetricDatabase : ViewModelBase
    {
        ...
        public static async void Initialize()
        {
            ...
            var result = await NAVOExtractionProgram.Execute(Globals.AppSettings.NAVOConfiguration.DBDBEXEPath, commandArgs, null);
        ...
        public static async Task<Bathymetry> ExtractAsync(string outputPath, string selectedResolution, GeoRect extractionArea)
        {
            ...
            var result = await NAVOExtractionProgram.Execute(Globals.AppSettings.NAVOConfiguration.DBDBEXEPath, commandArgs, outputDirectory);
        ...
        public static Bathymetry ExtractArea(NAVOBackgroundExtractor backgroundExtractor)
        {
            ...
            var result = NAVOExtractionProgram.Execute(Globals.AppSettings.NAVOConfiguration.DBDBEXEPath, commandArgs, backgroundExtractor.DestinationPath).Result;
            var resarray = result.Split('\n');
            foreach (var line in resarray.Where(line => line.Contains("ERROR"))) throw new ApplicationException("DigitalBathymetricDatabase: Error extracting requested area: " + line);
