...
namespace ESME.TransmissionLoss
{
    public class TransmissionLossCache : MemoryCache
    {
        public TransmissionLossCache(string name, NameValueCollection config) : base(name, config)
        {
            ...
            DefaultCacheItemPolicy.RemovedCallback += arguments =>
            {
                try
                {
                    Debug.WriteLine(string.Format("{0}: Removing radial ({1:0.###}, {2:0.###})/{3} from cache",
                                                  DateTime.Now,
                                                  ((Task<Radial>)arguments.CacheItem.Value).Result.TransmissionLoss.AnalysisPoint.Geo.Latitude,
                                                  ((Task<Radial>)arguments.CacheItem.Value).Result.TransmissionLoss.AnalysisPoint.Geo.Longitude,
                                                  ((Task<Radial>)arguments.CacheItem.Value).Result.Bearing));
                } catch (Exception) {}
            };
        }
