...
namespace HRC.Collections
{
        ...
        INotifyPropertyChanged
    {
        ...
        #region public
        public ObservableConcurrentDictionary() { KeyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>(); }

        public ObservableConcurrentDictionary(IEnumerable<KeyValuePair<TKey, TValue>> dictionary)
        {
            KeyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>();

            foreach (var entry in dictionary) DoAddEntry(entry.Key, entry.Value);
        }

        public ObservableConcurrentDictionary(IEqualityComparer<TKey> comparer) { KeyedEntryCollection = new KeyedDictionaryEntryCollection<TKey>(comparer); }

        public ObservableConcurrentDictionary(IEnumerable<KeyValuePair<TKey, TValue>> dictionary, IEqualityComparer<TKey> comparer)
        {
        ...
        #region protected
        protected ObservableConcurrentDictionary(SerializationInfo info, StreamingContext context) { _siInfo = info; }
        #endregion protected
        ...
        Dictionary<TKey, TValue> TrueDictionary
        {
            get
            {
                if (_dictionaryCacheVersion != _version)
                {
                    ...
                    foreach (var entry in KeyedEntryCollection) _dictionaryCache.Add((TKey)entry.Key, (TValue)entry.Value);
        ...
        public bool ContainsKey(TKey key) { return KeyedEntryCollection.Contains(key); }

        public bool ContainsValue(TValue value) { return TrueDictionary.ContainsValue(value); }

        public IEnumerator GetEnumerator() { return new Enumerator<TKey, TValue>(this, false); }

        public bool Remove(TKey key) { return DoRemoveEntry(key); }
        ...
        protected virtual bool ClearEntries()
        {
            ...
            var result = (Count > 0);
            if (result) // if so, clear the dictionary
        ...
        protected virtual void OnCollectionChanged(NotifyCollectionChangedEventArgs args) { if (CollectionChanged != null) CollectionChanged(this, args); }
        ...
        protected virtual bool SetEntry(TKey key, TValue value)
        {
            ...
            if (keyExists && value.Equals((TValue)KeyedEntryCollection[key].Value)) return false;

            // otherwise, remove the existing entry
            if (keyExists) KeyedEntryCollection.Remove(key);
        ...
        bool DoRemoveEntry(TKey key)
        {
            ...
            {
                _version++;
                if (index > -1) FireEntryRemovedNotifications(entry, index);
            }
        ...
        void FireEntryReplacedNotifications(DictionaryEntry entry, int index)
        {
            // fire the relevant PropertyChanged notifications
            //FirePropertyChangedNotifications();
            ...
            // fire CollectionChanged notification
            //OnCollectionChanged(index > -1
            //                        ? new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Replace, new KeyValuePair<TKey, TValue>((TKey)entry.Key, (TValue)entry.Value), index)
            //                        : new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset));
        }
        ...
        bool IDictionary<TKey, TValue>.Remove(TKey key) { return DoRemoveEntry(key); }

        bool IDictionary<TKey, TValue>.ContainsKey(TKey key) { return KeyedEntryCollection.Contains(key); }
        ...
        bool IDictionary.Contains(object key) { return KeyedEntryCollection.Contains((TKey)key); }

        IDictionaryEnumerator IDictionary.GetEnumerator() { return new Enumerator<TKey, TValue>(this, true); }
        ...
        bool ICollection<KeyValuePair<TKey, TValue>>.Contains(KeyValuePair<TKey, TValue> kvp) { return KeyedEntryCollection.Contains(kvp.Key); }

        void ICollection<KeyValuePair<TKey, TValue>>.CopyTo(KeyValuePair<TKey, TValue>[] array, int index)
        {
            if (array == null) throw new ArgumentNullException("CopyTo() failed:  array parameter was null");
            if ((index < 0) || (index > array.Length)) throw new ArgumentOutOfRangeException("CopyTo() failed:  index parameter was outside the bounds of the supplied array");
            if ((array.Length - index) < KeyedEntryCollection.Count) throw new ArgumentException("CopyTo() failed:  supplied array was too small");

            foreach (var entry in KeyedEntryCollection) array[index++] = new KeyValuePair<TKey, TValue>((TKey)entry.Key, (TValue)entry.Value);
        }
        ...
        bool ICollection<KeyValuePair<TKey, TValue>>.Remove(KeyValuePair<TKey, TValue> kvp) { return DoRemoveEntry(kvp.Key); }
        #endregion ICollection<KeyValuePair<TKey, TValue>>

        #region ICollection
        void ICollection.CopyTo(Array array, int index) { ((ICollection)KeyedEntryCollection).CopyTo(array, index); }
        ...
        IEnumerator<KeyValuePair<TKey, TValue>> IEnumerable<KeyValuePair<TKey, TValue>>.GetEnumerator() { return new Enumerator<TKey, TValue>(this, false); }
        ...
        public virtual void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            foreach (var entry in KeyedEntryCollection) entries.Add(entry);
        ...
        public virtual void OnDeserialization(object sender)
        {
            if (_siInfo != null)
            {
                ...
                foreach (var entry in entries) AddEntry((TKey)entry.Key, (TValue)entry.Value);
            }
        }
        ...
        {
            #region constructors
            internal Enumerator(ObservableConcurrentDictionary<TKey, TValue> dictionary, bool isDictionaryEntryEnumerator)
            {
            ...
            {
                if (_index == -1) throw new InvalidOperationException("The enumerator has not been started.");
                if (_index == -2) throw new InvalidOperationException("The enumerator has reached the end of the collection.");
            }

            void ValidateVersion() { if (_version != _dictionary._version) throw new InvalidOperationException("The enumerator is not valid because the dictionary changed."); }
            #endregion private
            ...
            #region fields
            readonly ObservableConcurrentDictionary<TKey, TValue> _dictionary;
        ...
        [NonSerialized] readonly SerializationInfo _siInfo;
        int _countCache;
