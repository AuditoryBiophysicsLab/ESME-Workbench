...
namespace DavesWPFTester.AxisLabeling.Renderer
{
    ...
    public class FrameLayer : Layer
    {
        ...
        public override void Layout(Rect screen)
        {
            ...
            AxisLayout xAxisLayout = new AxisLayout(false, X, X.Range, xRange,
                new Func<string, decimal, Axis, RectF>((label, pos, axis) => ComputeLabelRect(label, pos, bottomPanel.ToScreen(), screen, axis)), screen);
            Axis xAxis = xAxisLayout.layoutAxis();
            xRange = xAxis.visibleRange;
            
            AxisLayout yAxisLayout = new AxisLayout(true, Y, Y.Range, yRange,
                new Func<string, decimal, Axis, RectF>((label, pos, axis) => ComputeLabelRect(label, pos, leftPanel.ToScreen(), screen, axis)), screen);
            ...
            RectF bottomPanelSize = new LineMark(xRange.min, xRange.max, 0, 0).bounds(bottomPanel.ToScreen());
            RectF leftPanelSize = new LineMark(0, 0, yRange.min, yRange.max).bounds(leftPanel.ToScreen());
            ...
            if (DrawXAxis)
            {
                foreach (Tuple<decimal, string> kvp in xAxis.labels)
                {
                    ...
                    bottomPanel.Add(new LineMark(value, value, 0, -xAxis.tickSize).SetStrokeColor(Brushes.Black).SetStrokeWidth(0.5)); // the tick mark

                    if (DrawLabels)
                        bottomPanel.Add(new LabelMark(kvp.Item2, value, -xAxis.tickSize, Brushes.Black, ContentAlignment.TopCenter, true)
        ...
        static public RectF ComputeLabelRect(string label, decimal position, Transform transform, Rect screen, Axis data)
        {
            LabelMark text;
