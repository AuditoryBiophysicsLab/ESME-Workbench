...
namespace DavesWPFTester.AxisLabeling.Renderer
{
    public class PlotPanel : Panel
    {
        ...
        public PlotPanel(RenderState state)
        {
            ...
            Plot.SetStroke(Colors.Gray, 0.5).SetOrder(100000);
            ...
            {
                transform = p => new Point(p.X - (float)this.x0(), (float)this.y1() - p.Y),
                untransform = p => new Point(p.X + (float)this.x0(), (float)this.y1() + p.Y)
            };
            ...
            {
                transform = p => new Point((float)(p.X * Plot.Width()), (float)(p.Y * Plot.Height())),
                untransform = p => new Point((float)(p.X / Plot.Width()), (float)(p.Y / Plot.Height()))
            };
        ...
        public void render(Graphics g, Rect rect)
        {
            this.screen = rect;
            this.render(g, new Transform());
        }

        public IEnumerable<Mark> hit(Point p, Rect rect)
        {
            this.screen = rect;
            return this.hit(p, new Transform());
        }

        public IEnumerable<Mark> hit(Rectangle p, Rect rect)
        {
            this.screen = rect;
            return this.hit(p, new Transform());
        }
