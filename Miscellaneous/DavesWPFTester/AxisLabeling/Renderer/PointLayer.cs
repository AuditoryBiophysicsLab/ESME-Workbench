            ...
            if (x is Numeric || y is Numeric)
            {
                ...
                for (int i = 0; i < xp.Count(); i++)
                {
                    LabelMark label = new LabelMark(
                        ...
                        Colors.Black, ContentAlignment.BottomLeft)
                        .SetVisible(false)
                        .SetOrder(500);
                    labels.Add(label);
                ...
                foreach (var bin in bins)
                {
                    if (vertical)
                    {
                        points.Add(new BarMark(bin.loc.x - 0.4, bin.loc.y - 0.5, bin.loc.x + 0.4, bin.loc.y - 0.5 + (double)bin.cnt / max * 0.95)
                            .SetFillColor(blue));
                        labels.Add(new LabelMark("" + bin.cnt, bin.loc.x, bin.loc.y - 0.5 + (double)bin.cnt / max * 0.95, Colors.Black, ContentAlignment.TopCenter));
                    }
                    else
                    {
                        points.Add(new BarMark(bin.loc.x - 0.5, bin.loc.y - 0.4, bin.loc.x - 0.5 + (double)bin.cnt / max * 0.95, bin.loc.y + 0.4)
                            .SetFillColor(blue));
                        labels.Add(new LabelMark("" + bin.cnt, bin.loc.x - 0.5 + (double)bin.cnt / max * 0.95, bin.loc.y, Colors.Black, ContentAlignment.MiddleRight));
                    }
                }
            }                
