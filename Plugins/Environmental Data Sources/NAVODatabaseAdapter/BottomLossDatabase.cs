...
namespace NAVODatabaseAdapter
{
    public static class BottomLossDatabase
    {
        ...
        static string ExtractorArgument(bool isLowFrequency, Geo location, [NotNull]string lfblExePath, [NotNull]string hfblExePath)
        {
            var database = Path.Combine(isLowFrequency ? Path.GetDirectoryName(lfblExePath) : Path.GetDirectoryName(hfblExePath), "dbases/");
