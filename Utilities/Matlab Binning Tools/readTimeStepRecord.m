...
function timeStruct = readTimeStepRecord(fid,offset)
    % explicitly seek to the given record, which is probably slow. 
    status = fseek(fid,offset,'bof');
    assert(status ~= -1, ferror(fid));
    % then, just read the record
    timeStruct = readNextTimeStepRecord(fid);
end

%% read the next time step record; this assumes that fid is pointing to the beginning of a valid record.
function timeStruct = readNextTimeStepRecord(fid)
    % read the header
    [header,fid]=readTimeStepHeader(fid);
    % read the rest
...
%% read the header of a timesteprecord
function [header,fid] = readTimeStepHeader(fid)
