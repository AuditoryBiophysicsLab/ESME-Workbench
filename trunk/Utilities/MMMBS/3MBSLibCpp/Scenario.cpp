...
int CScenario::GetSoundSourceTypeCount()
{
	...
	_ASSERT(VerifySoundSourceListPlacement());
	_ASSERT(VerifySoundSourceModelsLoadedCount());
	return m_mbSce.numAcstSrcTypes;
}
...
int CScenario::GetTotalSoundSourceCount()
{
	...
	return m_mbSce.totalNumAcstcSrcs;
}
...
int CScenario::GetNthSoundSourceModelCount(int Nth)
{
	...
	return pSpe->GetTotalAnimatCount();
}
...
ANIMATSTATE CScenario::RetrieveFirstAnimatStateAtIndex(DWORD Index)
{
	...
	if(m_firstAnimatStateHoldArray == NULL /*|| Index >= m_animatStateHoldArrayLength*/)
		return as;
	return m_firstAnimatStateHoldArray[Index];
}
...
{
	SCENARIOPARAMS sce = m_mbSce;
	if(pUser != NULL)
		sce.user = *pUser;
	return m_fileMgr.GetMemoryAllocationDetails(sce);
}
...
void CScenario::_RunScenario()
{
	...
	C3MBRandom mbRandom;
	TCHAR szMessageBuffer[SIZE_128];
...
{
	return m_fileMgr.GetWorkingList(StartTime, Duration, &m_mbSce.intrvlOutptLim);
}
...
{
	return AddSpecies(ofn->lpstrFile, Index);
}
...
int CScenario::GetPodCount(int SpeciesIndex)
{
	...
	return (m_speciesList.Get(SpeciesIndex))->GetNumberOfPods();
}
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetNumberOfIndividuals();
}
...
int CScenario::GetAnimatCount()
{
	...
	return m_mbSce.totalNumAnimats;
}
...
CSpecies *CScenario::GetSpeciesAtIndex(int Index){ return m_speciesList.Get(AsserteSpeciesIndex(Index));}
int CScenario::GetAnimatCount(int SpeciesIndex){ return GetSpeciesAtIndex(SpeciesIndex)->GetTotalAnimatCount();}

int CScenario::GetPodMemberCount(int SpeciesIndex, int PodIndex)
{
	return GetSpeciesAtIndex(SpeciesIndex)->GetNumberAnimatsInPod(PodIndex);
}
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetPodFocalCoordinate(PodIndex);
}

double CScenario::GetPodLeaderFocalDistance(int SpeciesIndex, int PodIndex)
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetPodLeaderFocalDistance(PodIndex);
}
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetShoreFollowingDepth();
}

double CScenario::GetMinimumSeededingDepth(int SpeciesIndex)
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetMinimumSeedingDepth();
}
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetPodLeaderType(PodIndex);
}
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->GetNumBehaviorsModeled();
}
...
int CScenario::SetAcousticSrceLimitOutput(BOOL Limit)
{
	...
	ResetAcousticSourceInf();
	return m_mbSce.numSaveIterations = m_fileMgr.GetStateSaveToFileCount(m_mbSce.startTime, m_mbSce.duration, &m_mbSce.intrvlOutptLim);
}

BOOL CScenario::GetAcousticSrceLimitOutput()
{
	return m_mbSce.acousticPingCycleOutputLimit;
}

BOOL CScenario::CSVFileLoaded()
{
	return m_fileMgr.CSVFileLoaded();
}
...
{
	return m_mbSce.intrvlOutptLim.enabled;
}
...
{
	return m_staticLib.Time_ToHrMinSec(m_mbSce.duration);
}


int CScenario::GetDurationSeconds()
{
	return m_mbSce.duration;
}
...
{
	return m_staticLib.Time_ToHrMinSec(m_mbSce.startTime);
}
...
RESLT CScenario::SeedingCoordinateIsValid(UINT SpeciesIndex, double Lat, double Lon)
{
	...
	double bathyDepthAtLatLon;
	AsserteSpeciesIndex(SpeciesIndex);

	//_ASSERT(0);

	if(SpeciesIndex >= m_speciesList.Length())
...
RESLT CScenario::LoadBathymetryFromTextFile(OPENFILENAME *Ofn)
{
	...
	if(strcmp(szBuff, ".txt") == 0)
		isText = TRUE;
	return m_envData.bathymetry.LoadFromTextFile(Ofn, isText);
}
RESLT CScenario::LoadBathymetryFromTextFile(TCHAR *FileName)
{
	...
	if(strcmp(szBuff, ".txt") == 0)
		isText = TRUE;
	return m_envData.bathymetry.LoadFromTextFile(FileName, isText);
}
...
{
	return m_envData.bathymetry.IsDataLoaded();
}
...
RESLT CScenario::LoadSalinityFromTextFile(OPENFILENAME *Ofn)
{
	...
	return m_envData.salinity.LoadFromTextFile(Ofn, isText);
}
RESLT CScenario::LoadSalinityFromTextFile(TCHAR *FileName)
{
	...
	return m_envData.salinity.LoadFromTextFile(FileName, isText);
}
...
{
	return m_envData.salinity.IsDataLoaded();
}
...
RESLT CScenario::LoadTemperatureFromTextFile(OPENFILENAME *Ofn)
{
	...
	return m_envData.temperature.LoadFromTextFile(Ofn, isText);
}
RESLT CScenario::LoadTemperatureFromTextFile(TCHAR *FileName)
{
	...
	return m_envData.temperature.LoadFromTextFile(FileName,isText);
}
/*******************************************************************************
...
{
	return m_envData.temperature.IsDataLoaded();
}
...
{
	return m_fileMgr.SaveScenario(FileName, &m_mbSce, &m_envData, &m_speciesList);
}
...
void CScenario::CommandPromptOutput(TCHAR *sz){	printf(sz);}

CBathymetry *CScenario::GetBathymetryClassRef(){ return &m_envData.bathymetry;}
...
{
	return m_envData.bathymetry.GetValueAtCoordinate(lat, lon);
}
ENVDATAPOINTCOUNT CScenario::GetBathymetryDataPointCounts(){ return m_envData.bathymetry.GetDataPointCounts(FALSE);}
BATHYEXTREMES CScenario::GetBathymetryExtremes(){ return m_envData.bathymetry.GetExtremes();}
double CScenario::GetTotalBathySufaceAreaMeters(){	return m_envData.bathymetry.GetTotalSufaceAreaMeters();}
double CScenario::GetBathymetryLandSufaceAreaMeters(){	return m_envData.bathymetry.GetLandSufaceAreaMeters();}
double CScenario::GetBathymetryWaterSufaceAreaMeters(){	return m_envData.bathymetry.GetWaterSurfaceAreaMeters();}
...
INHABITINF CScenario::GetPodMemberInitialCoordinate(int SpeciesIndex, int PodIndex, int PodMemberIndex)
{
	AsserteSpeciesIndex(SpeciesIndex);
	m_mbsActiveMutex.Lock();
	INHABITINF coord = (m_speciesList.Get(SpeciesIndex))->GetPodMemberInitialCoordinate(PodIndex, PodMemberIndex);
	m_mbsActiveMutex.Unlock();
	return coord;
}
...
INHABITINF CScenario::GetIndividualInitialCoordinate(int SpeciesIndex, int IndividualIndex)
{
	...
	m_mbsActiveMutex.Lock();
	INHABITINF coord = (m_speciesList.Get(SpeciesIndex))->GetIndividualInitialCoordinate(IndividualIndex);
	m_mbsActiveMutex.Unlock();
...
INHABITINF CScenario::GetIndividualCoordinate(int SpeciesIndex, int IndividualIndex)
{
	...
	m_mbsActiveMutex.Lock();
	INHABITINF coord = (m_speciesList.Get(SpeciesIndex))->GetIndividualCoordinate(IndividualIndex); 
	m_mbsActiveMutex.Unlock();
...
INHABITINF CScenario::GetPodMemberCoordinate(int SpeciesIndex, int PodIndex, int PodMemberIndex)
{
	...
	m_mbsActiveMutex.Lock();
	INHABITINF coord = (m_speciesList.Get(SpeciesIndex))->GetPodMemberCoordinate(PodIndex, PodMemberIndex);
	m_mbsActiveMutex.Unlock();
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->ModelToText(fd);
}
...
{
	AsserteSpeciesIndex(SpeciesIndex);
	return (m_speciesList.Get(SpeciesIndex))->ModelToText(FileName);
}
...
{
	_ASSERT(m_mbSce.numSaveIterations == (DWORD)m_fileMgr.GetStateSaveToFileCount(m_mbSce.startTime, m_mbSce.duration, &m_mbSce.intrvlOutptLim));
	return m_mbSce.numSaveIterations;
}
