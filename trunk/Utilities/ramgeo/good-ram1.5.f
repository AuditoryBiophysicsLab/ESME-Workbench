...
c
C      parameter (mr=100,mz=8000,mp=10)
      parameter (mr=100,mz=100000,mp=10)
...
c dmf 14may03 - code patterned after collin's own 
c   to implement simple source steering
c   note: assumes source depth is at the top of the array
      real spacing, steer_ang
      complex ele_wgt(64)
      integer num_ele,status
      num_ele = 1
      spacing = 1.
      steer_ang = 0.
      ele_wgt(1) = 1	
      open(unit=4,status='old',file='sra.in',form='formatted',
     >     IOSTAT=status)

      IF (status==0) THEN
        !write(*,*)'Using source weighting file sra.in'
        read(4,*) num_ele, spacing, steer_ang
        !write(*,*)'num_ele=',num_ele,' spacing=',spacing
        !write(*,*)'steer_ang=',steer_ang
        Do ii = 1,num_ele
          read(4,*) dummy, ele_wgt(ii)
	  !write(*,*)dummy, ' ele_wgt(',ii,')=',ele_wgt(ii) 
        End Do
        close (4)
      ELSE
        write(*,*)'No sra.in file, omni directional source is assumed!'
      END IF

      ci=cmplx(0.0,1.0)
      SINT=SIN(steer_ang*PI/180.0)
      zss=zs
      Do iii=1,num_ele
          si=1.0+zss/dz
          is=ifix(si)
          dis=si-float(is)
          FACT=ELE_WGT(III)*CEXP(CI*K0*ZSS*SINT)
          u(is)=(1.0-dis)*fact*sqrt(2.0*pi/k0)/(dz*alpw(is))
          u(is+1)=dis*fact*sqrt(2.0*pi/k0)/(dz*alpw(is))
          zss=zss+spacing
      End Do
